...
$n = 1{,}441{,}792$ (937\,ms), which is the largest $k = n$ problem
size that fits reliably within one second on this instance.
% TODO(final-numbers): frontier re-measurement pending (E2/E3)
...
Two issues are under active resolution on the current B200 instance.
First, at $n \ge 1{,}048{,}576$ with $k = n$, the implementation runs
out of memory: the FFT-stride layout and arena-allocated polynomial
buffers require ${\sim}180$\,GB at this size, exceeding the available
HBM3e capacity. Second, at $n = 524{,}288$ with $k = n$, cuFFT plan
creation fails with \texttt{CUFFT\_INTERNAL\_ERROR} (code~5). This is
likely a cuFFT internal resource exhaustion bug at this specific
problem shape; a workaround using a composite of smaller batched plans
is being evaluated. Both issues are explicitly marked for resolution
in the E2/E3 measurement cycle.
