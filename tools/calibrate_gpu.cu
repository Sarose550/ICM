...
static int write_header(const char *path,
                        ...
                        int sm_count) {
    /* Carry over tables layered onto an existing header by SEPARATE tools
     * (the B-selection sweep: tools/calibrate_gpu_best_b.cu +
     * tools/splice_calib_points.py).  This tool cannot re-measure them, and
     * silently dropping them on recalibration is the raw-`cp` data
     * destruction trap tools/extend_calib_sizes.py exists to prevent on the
     * CPU side.  Read them out before truncating the file. */
    std::string carried_bselect, carried_uncalib;
    {
        FILE *old_f = fopen(path, "r");
        if (old_f) {
            std::string old_text;
            char buf[65536];
            size_t got;
            while ((got = fread(buf, 1, sizeof buf, old_f)) > 0)
                old_text.append(buf, got);
            fclose(old_f);
            size_t b = old_text.find("#define GPU_N_BSELECT_POINTS");
            if (b != std::string::npos) {
                size_t arr = old_text.find("gbselect_B", b);
                size_t e = (arr == std::string::npos)
                             ? std::string::npos : old_text.find("};", arr);
                if (e != std::string::npos)
                    carried_bselect = old_text.substr(b, e + 2 - b);
            }
            size_t u = old_text.find("#define GPU_UNCALIB_NS_PER_POINT");
            if (u != std::string::npos) {
                size_t eol = old_text.find('\n', u);
                if (eol != std::string::npos)
                    carried_uncalib = old_text.substr(u, eol - u);
            }
        }
    }
    FILE *f = fopen(path, "w");
    ...
    if (!carried_uncalib.empty())
        fprintf(f, "%s\n", carried_uncalib.c_str());
    if (!carried_bselect.empty()) {
        fprintf(f, "\n/* B-selection table carried over from the previous header (measured by\n"
                   " * tools/calibrate_gpu_best_b.cu, spliced by tools/splice_calib_points.py;\n"
                   " * this tool does not measure it).  Re-run that sweep if FFT timings\n"
                   " * changed materially. */\n%s\n", carried_bselect.c_str());
    } else {
        fprintf(f, "\n/* No B-selection calibration yet: empty table, the planner falls back\n"
                   " * to B=64 (gpu_empirical_best_B's GPU_N_BSELECT_POINTS == 0 guard).\n"
                   " * Layer real data in with tools/calibrate_gpu_best_b.cu +\n"
                   " * tools/splice_calib_points.py. */\n"
                   "#define GPU_N_BSELECT_POINTS 0\n"
                   "static const int gbselect_n[1] = {0};\n"
                   "static const int gbselect_k[1] = {0};\n"
                   "static const int gbselect_B[1] = {0};\n");
    }
    fprintf(f, "\n#endif\n");
    fclose(f);
    return 1;
...
int main(int argc, char **argv) {
    /* No silent default output path: the same rule as the CPU build's
     * DEVICE= (writing devices/b200/ by default would clobber the shipped
     * B200 data when run on some other card). */
    if (argc < 2) {
        fprintf(stderr,
                "usage: %s devices/<gpu_device>/gpu_fft_config.h [max_size] [--quick]\n"
                "  <gpu_device>: output device directory (b200 for the shipped B200 data;\n"
                "                create a new directory for a new card)\n"
                "  max_size:     FFT calibration ceiling (default 131072)\n",
                argv[0]);
        return 2;
    }
